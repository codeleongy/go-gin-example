...
import (
	...
	"github.com/leong-y/go-gin-example/service/cache_service"
)
...
func (t *Tag) ExistByName() (bool, error) {
	return models.ExisitTagByName(t.Name)
}

func (t *Tag) ExistByID() (bool, error) {
	return models.ExisitTagByID(t.ID)
}
...
func (t *Tag) Export() (string, error) {
	...
	f := excelize.NewFile()
	sheet1 := f.NewSheet("标签信息")
	if err != nil {
		return "", err
	}

	titles := []string{"ID", "名称", "创建人", "创建时间", "修改人", "修改时间"}
	f.SetSheetRow(f.GetSheetName(sheet1), "A1", &titles)
	for i, v := range tags {
		values := []string{
			strconv.Itoa(v.ID),
			v.Name,
		...
		f.SetSheetRow(f.GetSheetName(sheet1), fmt.Sprintf("A%d", i+2), &values)
	}
